...
export function AnalyticsChartCard({ title, description, summary, action, children, footer, className, chartClassName }: {
  ...
  return <section className={cn(overviewPanel, 'p-5', className)}>
    <header className="mb-5 flex flex-wrap items-start justify-between gap-3">
      <div className="min-w-0"><h2 id={titleId} className="text-base font-semibold tracking-tight">{title}</h2>{description ? <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{description}</p> : null}</div>
      {action}
    </header>
