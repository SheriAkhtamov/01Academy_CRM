...
import { KPI_ROLES, isFullCycleKpiRole, type KpiPlanVersion, type KpiRole } from '@shared/sales-kpi';
...
export function KpiSettingsPanel() {
  ...
  return <div className="space-y-7">
    <section aria-label={t('kpiEmployeePlans')} className="space-y-4">
      ...
      {query.isPending ? <div className="grid gap-4 lg:grid-cols-2">{KPI_ROLES.map((role) => <div key={role} className={`h-80 animate-pulse rounded-xl bg-muted ${isFullCycleKpiRole(role) ? 'lg:col-span-2' : ''}`} aria-busy="true" />)}</div>
        : query.isError ? <div role="alert" className={`${overviewPanel} flex flex-wrap items-center justify-between gap-3 p-5 text-sm`}><p>{t('failedToLoadData')}</p><button type="button" className={overviewButton} onClick={() => query.refetch()}>{t('retry')}</button></div>
          : <div className="grid gap-5 lg:grid-cols-2">{KPI_ROLES.map((role) => {
            ...
            return <article key={role} className={`${overviewPanel} flex flex-col ${isFullCycleKpiRole(role) ? 'lg:col-span-2' : ''}`} aria-label={t(roleKeys[role])}>
