...
ALTER TABLE academy_sales_kpi_plans
  ...
  CHECK (role IN ('hunter', 'closer', 'full_cycle', 'full_cycle_3500'));
...
-- Correct the first full-cycle plan to one shared fixed salary. Keep its
-- phase targets and bonuses, then clone those rules for the 3.5m variant.
...
SELECT 'full_cycle', to_char(now() AT TIME ZONE 'Asia/Tashkent', 'YYYY-MM'),
  source.config || jsonb_build_object(
    'baseSalaryUzs', 3000000,
    'baseSalaryMode', CASE
      WHEN COALESCE(source.config->'hunter'->>'baseSalaryMode', 'guaranteed') = 'guaranteed'
        AND COALESCE(source.config->'closer'->>'baseSalaryMode', 'guaranteed') = 'guaranteed'
      THEN 'guaranteed' ELSE 'conditional' END
  ), source.created_by
FROM LATERAL (
  SELECT config, created_by FROM academy_sales_kpi_plans
  WHERE role = 'full_cycle'
    AND effective_month <= to_char(now() AT TIME ZONE 'Asia/Tashkent', 'YYYY-MM')
  ORDER BY effective_month DESC, id DESC LIMIT 1
) source;
INSERT INTO academy_sales_kpi_plans (role, effective_month, config, created_by)
SELECT 'full_cycle_3500', to_char(now() AT TIME ZONE 'Asia/Tashkent', 'YYYY-MM'),
  source.config || jsonb_build_object('baseSalaryUzs', 3500000), source.created_by
FROM LATERAL (
  SELECT config, created_by FROM academy_sales_kpi_plans
  WHERE role = 'full_cycle'
    AND effective_month <= to_char(now() AT TIME ZONE 'Asia/Tashkent', 'YYYY-MM')
...
BEGIN
  ...
  VALUES (lead.id, CASE WHEN assigned_role IN ('hunter', 'full_cycle', 'full_cycle_3500') THEN lead.manager_id END,
...
BEGIN
  ...
  IF attributed_closer IS NULL AND academy_kpi_employee_role(assigned_manager) IN ('closer', 'full_cycle', 'full_cycle_3500') THEN
...
BEGIN
  ...
  INSERT INTO academy_sales_kpi_surveys (survey_id, closer_id)
    VALUES (NEW.id, COALESCE((SELECT closer_id FROM academy_sales_kpi_leads WHERE lead_id = linked_lead),
      CASE WHEN academy_kpi_employee_role(assigned_manager) IN ('closer', 'full_cycle', 'full_cycle_3500') THEN assigned_manager END))
    ON CONFLICT DO NOTHING;
  RETURN NEW;
END
$$;
